...
def append_to_console(message):
    console_output.configure(state='normal')  # Temporarily make the widget writable
    console_output.insert(tk.END, message + '\n')  # Add text
    console_output.configure(state='disabled')  # Disable editing again
    console_output.see(tk.END)

def update_frame():
    ret, cv_image = cap.read()  # Capture frame-by-frame
    if ret:
        cv_image = cv2.cvtColor(cv_image, cv2.COLOR_BGR2RGB)  # Convert from BGR to RGB
        pil_image = Image.fromarray(cv_image)
        tk_image = ImageTk.PhotoImage(image=pil_image)
        canvas.image = tk_image  # Keep a reference, prevent garbage-collection
        canvas.itemconfig(image_on_canvas, image=tk_image)
    main.after(10, update_frame)  # Call this function again in 10 ms
...
canvas.pack(side=tk.LEFT, padx=20, pady=10)  # Aligned to the left with padding
...
console_output.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=(0, 20))  # Fill horizontally
...
update_frame()  # Start the camera update loop
